...
class HomeSearchMobileLayout extends StatefulWidget {
  ...
  void Function({required int imageIndex,
...
class _HomeSearchMobileLayoutState extends State<HomeSearchMobileLayout> {
  ...
  bool isFiltersSelected = true; // It determines which form to show when sliding the panel
  bool isPanelOpened = false;
  UiEventsManager uiEventsManager = UiEventsManager();
  bool isLoading = false; // it is used when the user modifies the filters or the sorting
      ...
      return Scaffold(
        ...
        body: VAEABottomSheet(
          ...
          slidingPanel: Padding(
            ...
            child: (isFiltersSelected) ? buildFilterForm() : buildSortingForm(),
          ),
          panelController: _panelController
        ),
        bottomNavigationBar: (isPanelOpened) ? null : BottomNavigation(currentIndex: 0,),
      );
    });
  }
        ...
        Expanded(
          child: Stack(
            children: [
              ...
              Positioned( // filters and sortings
  ...
  Widget buildTabsSection() {
    return Container(
      ...
      child: VAEASegmentedButton<bool>(
          breakpoint: breakpoint,
          layoutConstraints: layoutConstraints,
          options: [AppLocalizations.of(context)!.listTabText, AppLocalizations.of(context)!.mapsTabText],
    ...
    return Container(
      ...
      child: GoogleMap(
        ...
        markers: (widget.listings == null)
            ? {}
            :  widget.listings!.map((e) => buildMarker(e.listingId, e.lat, e.lon, e.imagesUrls)).toSet(),
  ...
  Marker buildMarker(int listingId, double listingLat, double listingLon, List<String> imageUrls) {
    return Marker(
      markerId: MarkerId(listingId.toString()),
      position: LatLng(listingLat, listingLon),
      onTap: () { // take the user to the details page
  ...
  Widget buildHomesList() {
    ...
    } else if (widget.listings == null || widget.listings!.length == 0) {
  ...
  Future<void> handleSubmitFilterForm({
    ...
    widget.handleSubmitFilterForm(selectedHomeType: selectedHomeType,
