...
class SignUpValidator {
  static const String _emailRegExpStr =
      r"^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,253}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,253}[a-zA-Z0-9])?)*$";
  static final RegExp _emailRegExp = RegExp(_emailRegExpStr);
  static const String _nameRegExpStr = r"^[\p{L} ,.'-]*$";
  static final RegExp namesRegExp =
      RegExp(_nameRegExpStr, unicode: true, caseSensitive: false, dotAll: true);
  ...
  String? validateFirstName(String? input) {
    ...
    } else if (!namesRegExp.hasMatch(input)) {
  ...
  String? validateConfirmPassword(
      String? passwordInput, String? confirmPasswordInput) {
    if (confirmPasswordInput == null ||
        confirmPasswordInput.replaceAll(" ", "").length == 0) {
      return AppLocalizations.of(context)!.requiredFieldErrorMsg;
  ...
  Future<String?> validateOTP(String? input, String emailAddress) async {
    try {
      VerifyOTPDto requestDto =
          VerifyOTPDto(emailAddress: emailAddress, otpCode: input!);
      if (input == null || input.replaceAll(" ", "").length == 0) {
        return AppLocalizations.of(context)!.requiredFieldErrorMsg;
