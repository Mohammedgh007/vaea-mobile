...
class RequestsContainer {
  ...
  static Future<Map<String, dynamic>> getData(String pathStr, Map<String, dynamic>? queryMap) async {
    try {
      // prepare the request uri
      Uri url;
      if (queryMap != null && queryMap.isNotEmpty) {
        String base = "${Env.apiUrl.substring(Env.apiUrl.indexOf("/") + 2)}:${Env.apiPort}"; // to remove https://
        url = Uri.http(base, pathStr, queryMap.map((key, value) => MapEntry(key, value.toString())));
      } else {
  ...
  /// @pre-condition the caller checks internet connection.
  /// @throws UnknownException
  static Future<Map<String, dynamic>> postData(String pathStr, Map<String, dynamic> payload) async {
    try {
  ...
  /// @pre-condition the caller checks internet connection.
  /// @throws UnknownException
  static Future<Map<String, dynamic>> postFormData(String pathStr, Map<String, dynamic> payload) async {
    try {
      // setup the header and uri
      Map<String, String> header = {
        "Content-Type": "application/json"
      ...
      debugPrint("after api post $pathStr ${response.data.toString()} ${formDataPayload}");
      if (response.statusCode == 200) {
      ...
      debugPrint("in catch $e");
      throw UnknownException(msg: "unknown error in RequestContainer.postFormData $e");
    }
  }
  ...
  static MultipartFile? mapFileToMultipartFile(File file) {
    ...
    return MultipartFile.fromFileSync(file.path, contentType: MediaType(fileType, fileSubType));
  }
